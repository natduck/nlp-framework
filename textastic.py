...
from parser import textParse, textClean
...
class Textastic:
  ...
  def load_text(self, filename, label=None, parser=None):
    """
        ...
        :param label: Label of the file contents
        :param parser:
        :return:
        """
    if label is None:
    ...
    with open(filename, "r", encoding='utf8', errors="ignore") as f:
      contents = f.read()
      self.file_contents[label] = textParse(contents)
      self.text_data["word_count"][label] = len(
        self.file_contents[label].split())

      afinn = Afinn()
      self.text_data["sentiment_score"][label] = afinn.score(
        self.file_contents[label])
  ...
  def load_stop_words(self, stopfile=""):
    ...
    if stopfile != "":
      with open(stopfile, "r") as stop_f:
        ...
        stop_f_words = textParse(stop_f_words)
  ...
  def wordcloud(self):
    ...
    for i, (key, value) in enumerate(self.file_contents.items()):
      ...
      wc = WordCloud(stopwords=self.stop_words,
  ...
  def sentiment_bar(self):
    """
        Creates a bar chart showing the sentiment scores in descending order.
        """
    df_scores = pd.DataFrame(self.text_data["sentiment_score"].items(),
                             columns=["label", "sentiment_score"])
    df_scores.sort_values("sentiment_score", ascending=False, inplace=True)
    return px.bar(data_frame=df_scores,
                  x="label",
                  y="sentiment_score",
                  title="Sentiment Score by Text")
